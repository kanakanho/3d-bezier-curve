...
struct ExternalStroke: Codable {
    ...
    public init(points: [SIMD3<Float>] = [], color: NSColor = .white, initPoint: SIMD3<Float> = .zero, maxRadius: Float = 1E-2) {
        // initPointからの相対位置に変換
...
// strokes.json から [ExternalStroke] を読み込み
let fileURL = URL(fileURLWithPath: FileManager.default.currentDirectoryPath).appendingPathComponent("strokes.json")
guard let data = try? Data(contentsOf: fileURL)
else {
    print("[3D] strokes.json の読み込みに失敗しました")
    exit(1)
}
let jsonDecoder = JSONDecoder()
jsonDecoder.keyDecodingStrategy = .convertFromSnakeCase
guard let externalStrokes = try? jsonDecoder.decode([ExternalStroke].self, from: data)
else {
    print("[3D] strokes.json のデコードに失敗しました")
    exit(1)
}

let input3Ds = externalStrokes.map { stroke in
    stroke.points.map { SIMD3<Double>(Double($0.x), Double($0.y), Double($0.z)) }
}

// original の 保存
let originalPoints3D: [BezierPoint3D] = input3Ds.flatMap { $0 }.map { BezierPoint3D($0) }
...
do {
    ...
    let fileURL = URL(fileURLWithPath: FileManager.default.currentDirectoryPath).appendingPathComponent("cubic_segments_3d_curvature_\(i).json")
